...
{
    /*
     * Helper methods for generating form elements
    */
...
<?php
    ...
    public function generateCheckbox($options, $field, $classes = '', $excludes = array()) {
        ...
        $has_options = (!empty(get_option($field))) ? get_option($field) : '';
        $checked = '';
        ...
        if(is_array($options)){
            ...
            $classes = (isset($classes)) ? $classes : 'inline-options';
            
            foreach($options as $k => $option) {
                
                $id = $field .'_' .$option;
                $label = str_replace('_', ' ', $option);
                $checked = (!empty($has_options) && !empty($has_options[$option])) ? 'checked' : '';
                
?>
                <label for="<?php echo $field .'_' . $option?>" class="<?php echo $classes ?>">
                    <?php $this->generateToggle($id, $checked); ?>
                    <input type="checkbox" name="<?php echo $field ?>[<?php echo $option ?>]" id="<?php echo $id ?>" <?php echo $checked ?>> <?php echo ucfirst(trim($label)) ?>
                </label>
...
<?php
            ...
            $option = $options;
?>
            <label for="<?php echo $field .'_' . $option?>">
                <input type="checkbox" name="<?php echo $fild ?>[<?php echo $option ?>]" id="<?php echo $field .'_' . $option?>"> <?php echo ucfirst(trim($option)) ?>
            </label>
<?php
    ...
    public function generateInputBox($options, $field, $title='', $desc='', $classes='') {
        ...
        foreach($options as $k => $option){
            ...
            $saved_option = (!empty($has_options[$option])) ? $has_options[$option] : '';
...
?>
                ...
                <p class="description"><?php echo trim($desc) ?></p>
            </label>
<?php
...
?>
        <p>Select one ore more post types where you want to enable CommentRefs. <br>
